...
### 目的
塗装作業における色調整を支援するWebアプリケーション。画像から色を抽出し、目標色に近づけるための塗料混合比率を科学的に計算する。Webブラウザでの利用(PC,スマホ)を中心とした設計。
...
#### 5.1 画像交換操作
- **交換ボタン**: 色調コントローラと塗料混合セクションの間に配置
- **ワンクリック交換**: 単一ボタンで画像A・Bの完全交換
- **レスポンシブ対応**: PC・モバイル両環境での適切な表示
- **中央配置**: 左右幅100%、ボタン中央配置、上下余白0

#### 5.2 交換対象データ
- **画像プレビュー**: 画像A・Bのプレビュー表示を交換
- **色調整値**: 出発色・結果色の両方を交換
- **ファイルパス**: PATH入力フィールドの値を交換
- **状態同期**: 交換後の即座な画面更新
